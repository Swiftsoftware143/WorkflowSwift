...
pub async fn create_tag(
    ...
    let tenant_id = Uuid::parse_str(&claims.aid).map_err(|_| AppError::Unauthorized)?;
    features::enforce_feature_limit(&state.db, tenant_id, "max_tags", "Tags").await?;
    ...
    let existing = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM tags WHERE tenant_id = $1 AND name = $2",
    )
    .bind(tenant_id)
    .bind(&name)
    .fetch_one(&state.db)
    ...
    let tag = sqlx::query_as::<_, Tag>(
        r#"INSERT INTO tags (id, tenant_id, name, color) VALUES ($1, $2, $3, $4) RETURNING *"#,
    )
    .bind(Uuid::new_v4())
    .bind(tenant_id)
    .bind(&name)
...
pub async fn assign_tag(
    ...
    // Check tag exists and belongs to tenant
