...
pub async fn list_clients(
    ...
    let clients = sqlx::query_as::<_, Client>(
        "SELECT * FROM clients WHERE tenant_id = $1 AND is_active = true ORDER BY name ASC",
    )
    .bind(tenant_id)
    .fetch_all(&state.db)
    .await?;
...
pub async fn create_client(
    ...
    let tenant_id = Uuid::parse_str(&claims.aid).map_err(|_| AppError::Unauthorized)?;
    features::enforce_feature_limit(&state.db, tenant_id, "max_clients", "Clients").await?;
    ...
    let client = sqlx::query_as::<_, Client>(
        r#"INSERT INTO clients (id, tenant_id, name, email, phone, website, industry, notes)
...
pub async fn get_client(
    ...
    use sqlx::Row;
    let contacts_json: Vec<serde_json::Value> = contacts.iter().map(|r| {
        json!({
