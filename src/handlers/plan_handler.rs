...
pub async fn update_plan(
    ...
    if !claims.perm_is_super_admin.unwrap_or(false) {
        return Err(AppError::Forbidden(
            "Only the super admin can update plans".to_string(),
        ));
    }
    ...
    let price_monthly = req
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .or(existing.price_monthly);
    let price_yearly = req
        .get("price_yearly")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .or(existing.price_yearly);
    let features = req.get("features").or(existing.features.as_ref());
    let checkout_url = req
        .get("checkout_url")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .or(existing.checkout_url);
    let is_active = req
        .get("is_active")
    ...
    let payment_provider = req
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .or(existing.payment_provider.clone());
...
pub async fn delete_plan(
    ...
    if claims.role != "admin" && claims.role != "agency_admin" {
        return Err(AppError::Forbidden(
            "Only admins can delete plans".to_string(),
        ));
    }
...
pub async fn admin_list_all_plans(
    ...
    if claims.role != "agency_admin" && claims.role != "admin" {
        return Err(AppError::Forbidden(
            "Only admins can list all plans".to_string(),
        ));
    }
    let plans = sqlx::query_as::<_, PlanTier>(
...
pub async fn admin_update_plan(
    ...
    if claims.role != "agency_admin" && claims.role != "admin" {
        return Err(AppError::Forbidden(
            "Only admins can update plans".to_string(),
        ));
    }
    // Check plan exists
    ...
    let price_monthly = req
        .get("price_monthly")
        .and_then(|v| v.as_str())
        .or(existing.price_monthly.as_deref())
        .unwrap_or("0.00");
    let price_yearly = req
        .get("price_yearly")
        .and_then(|v| v.as_str())
        .or(existing.price_yearly.as_deref())
        .unwrap_or("0.00");
    let features = req
        .get("features")
        .cloned()
        .or_else(|| existing.features.clone());
...
pub async fn admin_get_plan(
    ...
    if claims.role != "agency_admin" && claims.role != "admin" {
        return Err(AppError::Forbidden(
            "Only admins can view plans".to_string(),
        ));
    }
    let plan = sqlx::query_as::<_, PlanTier>(
        "SELECT id, name, slug, description, price_monthly::text as price_monthly, price_yearly::text as price_yearly, features, checkout_url, is_active, sort_order, payment_provider, created_at FROM plan_tiers WHERE id = $1::uuid"
...
pub async fn admin_update_plan_features(
    ...
    let features = req
        .get("features")
        .ok_or_else(|| AppError::Validation("features object is required".to_string()))?;
