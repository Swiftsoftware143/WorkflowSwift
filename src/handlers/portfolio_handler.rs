...
pub async fn list_portfolio_companies(
    ...
    let rows = sqlx::query("SELECT id::text, name, slug, settings::text, created_at::text FROM portfolio_companies WHERE tenant_id = $1 ORDER BY name")
        .bind(tenant_id)
...
pub async fn create_portfolio_company(
    ...
    let tenant_id = Uuid::parse_str(&claims.aid).map_err(|_| AppError::Unauthorized)?;
    features::enforce_feature_limit(&state.db, tenant_id, "max_portfolio_companys", "Portfolio Companys").await?;
    ...
    let id = Uuid::new_v4();
    sqlx::query("INSERT INTO portfolio_companies (id, tenant_id, name, slug) VALUES ($1, $2, $3, $4)")
...
pub async fn get_portfolio_company(
    ...
    let row = sqlx::query("SELECT id::text, name, slug, settings::text, created_at::text FROM portfolio_companies WHERE id = $1 AND tenant_id = $2")
        .bind(company_id).bind(tenant_id)
...
pub async fn update_portfolio_company(
    ...
    if let Some(name) = req.get("name").and_then(|v| v.as_str()) {
        sqlx::query("UPDATE portfolio_companies SET name = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3")
            .bind(name).bind(company_id).bind(tenant_id).execute(&state.db).await?;
    }
    if let Some(slug) = req.get("slug").and_then(|v| v.as_str()) {
        sqlx::query("UPDATE portfolio_companies SET slug = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3")
            .bind(slug).bind(company_id).bind(tenant_id).execute(&state.db).await?;
    }
...
pub async fn internal_create_portfolio_company(
    ...
    sqlx::query(
        "INSERT INTO portfolio_companies (id, tenant_id, name, slug, email, description) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, slug = EXCLUDED.slug"
    )
    .bind(id)
    .bind(tenant_id)
    .bind(&name)
...
pub async fn delete_portfolio_company(
    ...
    sqlx::query("DELETE FROM portfolio_companies WHERE id = $1 AND tenant_id = $2")
        .bind(company_id).bind(tenant_id).execute(&state.db).await?;
