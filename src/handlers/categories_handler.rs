...
pub async fn list(
    ...
    let items = sqlx::query_as::<_, Item>(
        "SELECT id, tenant_id, name, created_at, updated_at FROM categories ORDER BY name LIMIT $1 OFFSET $2"
    )
...
pub async fn create(
    ...
    let item = sqlx::query_as::<_, Item>(
        "SELECT id, tenant_id, name, created_at, updated_at FROM categories WHERE id = $1"
    )
