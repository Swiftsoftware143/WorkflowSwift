...
pub async fn list(
    ...
    let items = sqlx::query_as::<_, Item>(
        "SELECT id, aid, name, created_at, updated_at FROM email_templates ORDER BY name LIMIT $1 OFFSET $2"
    )
...
pub async fn create(
    ...
    let item = sqlx::query_as::<_, Item>(
        "SELECT id, aid, name, created_at, updated_at FROM email_templates WHERE id = $1"
    )
