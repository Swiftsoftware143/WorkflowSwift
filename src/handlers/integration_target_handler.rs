...
pub async fn list_integration_targets(
    ...
    if let Some(pc_id) = portfolio_filter {
        let rows = sqlx::query("SELECT id::text, tenant_id::text, portfolio_company_id::text, user_id::text, name, provider, webhook_url, events::text, is_active, created_at::text FROM integration_targets WHERE tenant_id = $1 AND portfolio_company_id = $2 ORDER BY name")
            .bind(tenant_id).bind(pc_id)
    ...
    let rows = sqlx::query("SELECT id::text, tenant_id::text, portfolio_company_id::text, user_id::text, name, provider, webhook_url, events::text, is_active, created_at::text FROM integration_targets WHERE tenant_id = $1 ORDER BY name")
        .bind(tenant_id)
        .fetch_all(&state.db).await?;
    let targets: Vec<serde_json::Value> = rows.iter().map(|r| {
        json!({"id": r.try_get::<&str,_>("id").unwrap_or(""), "name": r.try_get::<&str,_>("name").unwrap_or(""), "provider": r.try_get::<&str,_>("provider").unwrap_or(""), "webhook_url": r.try_get::<&str,_>("webhook_url").unwrap_or(""), "is_active": r.try_get::<bool,_>("is_active").unwrap_or(false)})
...
pub async fn create_integration_target(
    ...
    let tenant_id = Uuid::parse_str(&claims.aid).map_err(|_| AppError::Unauthorized)?;
    features::enforce_feature_limit(&state.db, tenant_id, "max_integration_targets", "Integration Targets").await?;
    ...
    sqlx::query("INSERT INTO integration_targets (id, tenant_id, portfolio_company_id, user_id, name, provider, webhook_url, api_key) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)")
        .bind(id).bind(tenant_id).bind(pc_id).bind(user_id).bind(&name).bind(&provider).bind(&webhook_url).bind(api_key)
...
pub async fn update_integration_target(
    ...
    if let Some(name) = req.get("name").and_then(|v| v.as_str()) {
        sqlx::query("UPDATE integration_targets SET name = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3")
            .bind(name).bind(target_id).bind(tenant_id).execute(&state.db).await?;
    }
    if let Some(url) = req.get("webhook_url").and_then(|v| v.as_str()) {
        sqlx::query("UPDATE integration_targets SET webhook_url = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3")
            .bind(url).bind(target_id).bind(tenant_id).execute(&state.db).await?;
    }
    if let Some(active) = req.get("is_active").and_then(|v| v.as_bool()) {
        sqlx::query("UPDATE integration_targets SET is_active = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3")
            .bind(active).bind(target_id).bind(tenant_id).execute(&state.db).await?;
    }
...
pub async fn delete_integration_target(
    ...
    let tenant_id = Uuid::parse_str(&claims.aid).map_err(|_| AppError::Unauthorized)?;
    let target_id = Uuid::parse_str(&id).map_err(|_| AppError::BadRequest("Invalid ID".into()))?;
    sqlx::query("DELETE FROM integration_targets WHERE id = $1 AND tenant_id = $2")
        .bind(target_id).bind(tenant_id).execute(&state.db).await?;
