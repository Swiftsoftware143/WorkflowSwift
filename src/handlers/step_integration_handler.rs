...
pub async fn list_step_integrations(
    ...
    let step_id_str = params
        .get("step_id")
        .ok_or_else(|| AppError::BadRequest("step_id required".into()))?;
    let step_id =
        Uuid::parse_str(step_id_str).map_err(|_| AppError::BadRequest("Invalid step_id".into()))?;
