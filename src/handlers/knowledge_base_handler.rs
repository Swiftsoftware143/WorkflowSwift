...
pub async fn list(State(state): State<AppState>) -> ApiResult<Json<Value>> {
    ...
    let items = sqlx::query_as::<_, Item>(
        "SELECT id, aid, name, created_at, updated_at FROM knowledge_base ORDER BY name LIMIT $1 OFFSET $2"
    )
...
pub async fn create(
    ...
    let id = Uuid::new_v4();
    let aid = Uuid::nil();
    sqlx::query("INSERT INTO knowledge_base (id, aid, name) VALUES ($1, $2, $3)")
        .bind(id)
        .bind(aid)
        .bind(&body.name)
        .execute(&state.db)
    ...
    let item = sqlx::query_as::<_, Item>(
        "SELECT id, aid, name, created_at, updated_at FROM knowledge_base WHERE id = $1",
    )
    .bind(id)
    .fetch_one(&state.db)
    .await?;
