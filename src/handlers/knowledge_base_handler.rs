//! Knowledge Base handler — auto-generated.
...
pub async fn list(
    ...
    let items = sqlx::query_as::<_, Item>(
        "SELECT id, tenant_id, name, created_at, updated_at FROM knowledge_base ORDER BY name LIMIT $1 OFFSET $2"
    )
...
pub async fn create(
    ...
    sqlx::query("INSERT INTO knowledge_base (id, tenant_id, name) VALUES ($1, $2, $3)")
        .bind(id).bind(tenant_id).bind(&body.name)
        .execute(&state.db).await?;
    let item = sqlx::query_as::<_, Item>(
        "SELECT id, tenant_id, name, created_at, updated_at FROM knowledge_base WHERE id = $1"
    )
