...
pub async fn get_account_industry(
    ...
    for row in industry_rows {
        ...
        let name: String = row.try_get("industry_name").unwrap_or_else(|_| slug.clone());
...
pub async fn set_account_industry(
    ...
    let set_as_primary = req.get("set_as_primary")
    ...
    if is_new {
        ...
        if !limit_check.allowed && limit_check.limit != -1 {
            return Err(AppError::BadRequest(format!(
                ...
                current_count, limit_check.limit
            )));
        }
    }
...
pub async fn remove_account_industry(
    ...
    Ok(Json(json!({"message": format!("Industry '{}' dashboard removed", slug)})))
}
...
fn generate_industry_widgets(industry_slug: &str) -> Vec<(&'static str, String, serde_json::Value, serde_json::Value)> {
    ...
    let is_sales_pipeline = industry_slug.contains("lead") || industry_slug.contains("sales")
        || industry_slug.contains("recruit") || industry_slug.contains("agency");
    let is_project_tracker = industry_slug.contains("construction") || industry_slug.contains("development")
        || industry_slug.contains("flipping") || industry_slug.contains("publishing")
        || industry_slug.contains("education") || industry_slug.contains("training");
    let is_service_scheduler = industry_slug.contains("service") || industry_slug.contains("healthcare")
        || industry_slug.contains("wellness") || industry_slug.contains("professional");
    ...
    if is_sales_pipeline {
        widgets.push(("stat-counter", "Active Leads".to_string(),
            mk_config(&mk("active_leads"), "Leads / candidates in pipeline", None, None),
            pos(0, 0, 2, 1)));
        widgets.push(("stat-counter", "Conversion Rate".to_string(),
            mk_config(&mk("conversion_rate"), "Lead to close %", Some("%"), None),
            pos(0, 2, 2, 1)));
        widgets.push(("revenue-card", "Revenue".to_string(),
            mk_config(&mk("revenue"), "Total closed deals", None, Some("USD")),
            pos(0, 4, 3, 1)));
    } else if is_project_tracker {
        widgets.push(("stat-counter", "Active Projects".to_string(),
            mk_config(&mk("active_projects"), "In development / in progress", None, None),
            pos(0, 0, 2, 1)));
        widgets.push(("stat-counter", "Completed".to_string(),
        ...
        widgets.push(("stat-counter", "Active Clients".to_string(),
            mk_config(&mk("active_clients"), "Current clients / patients", None, None),
            pos(0, 0, 2, 1)));
        widgets.push(("stat-counter", "Appointments".to_string(),
            mk_config(&mk("appointments"), "Scheduled this week", None, None),
            pos(0, 2, 2, 1)));
        widgets.push(("revenue-card", "Billings".to_string(),
            mk_config(&mk("billings"), "Invoiced / collected", None, Some("USD")),
            pos(0, 4, 3, 1)));
    } else if is_ecommerce {
        widgets.push(("stat-counter", "Active Orders".to_string(),
            mk_config(&mk("active_orders"), "Orders in fulfillment", None, None),
            pos(0, 0, 2, 1)));
        widgets.push(("stat-counter", "Products".to_string(),
            mk_config(&mk("products"), "Active listings / inventory", None, None),
            pos(0, 2, 2, 1)));
        widgets.push(("revenue-card", "Sales".to_string(),
            mk_config(&mk("sales"), "Total revenue", None, Some("USD")),
            pos(0, 4, 3, 1)));
    } else if is_grant_funding {
        widgets.push(("stat-counter", "Active Grants".to_string(),
            mk_config(&mk("active_grants"), "In progress / pending", None, None),
            pos(0, 0, 2, 1)));
        widgets.push(("stat-counter", "Success Rate".to_string(),
            mk_config(&mk("success_rate"), "Awarded %", Some("%"), None),
            pos(0, 2, 2, 1)));
        widgets.push(("revenue-card", "Funding".to_string(),
            mk_config(&mk("funding"), "Total awarded", None, Some("USD")),
            pos(0, 4, 3, 1)));
    } else if is_govcon {
        widgets.push(("stat-counter", "Open Solicitations".to_string(),
            mk_config(&mk("solicitations"), "Matching your NAICS codes", None, None),
            pos(0, 0, 2, 1)));
        widgets.push(("stat-counter", "Bids Submitted".to_string(),
            mk_config(&mk("bids_submitted"), "This quarter", None, None),
            pos(0, 2, 2, 1)));
        widgets.push(("revenue-card", "Awarded Value".to_string(),
            mk_config(&mk("awarded"), "Total contract value", None, Some("USD")),
            pos(0, 4, 3, 1)));
    } else {
        // Generic fallback — works for ANY industry not explicitly matched
        widgets.push(("stat-counter", "Active Items".to_string(),
    ...
    if is_sales_pipeline {
        widgets.push(("stat-counter", "Follow-ups Due".to_string(),
            mk_config(&mk("followups_due"), "Tasks needing attention", None, None),
            pos(1, 0, 3, 1)));
        widgets.push(("stat-counter", "Warm Outreach".to_string(),
            mk_config(&mk("warm_outreach"), "Emails / calls sent today", None, None),
            pos(1, 3, 4, 1)));
    } else if is_project_tracker {
        widgets.push(("stat-counter", "Milestones Due".to_string(),
            mk_config(&mk("milestones_due"), "Upcoming deadlines", None, None),
            pos(1, 0, 3, 1)));
        widgets.push(("connection-status", "Integrations".to_string(),
            mk_config(&mk("integrations"), "API connections status", None, None),
            pos(1, 3, 4, 1)));
    } else if is_ecommerce {
        widgets.push(("stat-counter", "Abandoned Carts".to_string(),
            mk_config(&mk("abandoned_carts"), "Recovery opportunities", None, None),
            pos(1, 0, 3, 1)));
        widgets.push(("stat-counter", "Avg Order Value".to_string(),
            mk_config(&mk("avg_order"), "Per transaction", None, Some("$")),
            pos(1, 3, 4, 1)));
    } else if is_grant_funding {
        widgets.push(("stat-counter", "Applications Due".to_string(),
            mk_config(&mk("applications_due"), "Deadlines this month", None, None),
            pos(1, 0, 3, 1)));
        widgets.push(("stat-counter", "Donors / Funders".to_string(),
            mk_config(&mk("donors"), "Active relationships", None, None),
            pos(1, 3, 4, 1)));
    } else {
        // Generic secondary
        widgets.push(("stat-counter", "Pending Tasks".to_string(),
            mk_config(&mk("pending_tasks"), "Needing action", None, None),
            pos(1, 0, 3, 1)));
        widgets.push(("stat-counter", "Today's Activity".to_string(),
            mk_config(&mk("today_activity"), "Actions taken today", None, None),
            pos(1, 3, 4, 1)));
    }

    // Row 2: Activity feed (always present)
    widgets.push(("activity-feed", "Recent Activity".to_string(),
        mk_config(&mk("recent_activity"), "Latest events and updates", None, None),
        pos(2, 0, 7, 2)));

    // Row 3: Chart widget for trends (always present)
    widgets.push(("chart-trend", "Trends".to_string(),
...
fn mk_config(metric_key: &str, subtitle: &str, suffix: Option<&str>, currency: Option<&str>) -> Value {
    let mut m = serde_json::Map::new();
    m.insert("metric_key".to_string(), json!(metric_key));
    m.insert("subtitle".to_string(), json!(subtitle));
    if let Some(s) = suffix { m.insert("suffix".to_string(), json!(s)); }
    if let Some(c) = currency { m.insert("currency".to_string(), json!(c)); }
    Value::Object(m)
}
...
pub async fn get_dashboard_widgets(
    ...
    let industry_slug = match params.get("industry") {
        Some(slug) => {
            ...
            if !exists {
                return Err(AppError::NotFound(format!("Industry '{}' not found for this account", slug)));
            }
            slug.clone()
        }
        None => {
...
pub async fn push_widget_data(
    ...
    let metric_value = req.get("value")
    ...
    let dashboard_id: Uuid = if let Some(industry_slug) = req.get("industry_slug").and_then(|v| v.as_str()) {
