...
pub async fn trigger_n8n_workflow(
    ...
    let payload = req.get("payload").cloned().unwrap_or(serde_json::Value::Object(serde_json::Map::new()));
    ...
    let n8n_url = format!("{}/webhook/{}", state.config.n8n_webhook_url.trim_end_matches('/'), workflow_id);
...
pub async fn check_n8n_health(
    ...
    let client = reqwest::Client::new();
    let health_url = format!("{}/health", state.config.n8n_webhook_url.trim_end_matches('/'));

    match client.get(&health_url).send().await {
        Ok(resp) => {
            let status = resp.status().as_u16();
            let body: serde_json::Value = resp.json().await.unwrap_or(json!({"note": "health endpoint returned non-JSON"}));
